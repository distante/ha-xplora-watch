...
- All voice messages, videos and images are stored in `config/www/{voice|video|image|}`.
  - The voice message will be downloaded as amr and converted to mp3.
    - This conversion uses **ffmpeg**, run via Home Assistant's built-in `ffmpeg` integration (declared as a dependency, so it is set up automatically). Home Assistant OS, Supervised, and the official Container image already bundle the `ffmpeg` binary, so there is nothing to install. On a bare **Core** (venv/pip) install, make sure `ffmpeg` is available on the system `PATH`. If it is missing, only the voice→mp3 conversion is skipped (an error is logged) — text messages, images and videos are unaffected.
